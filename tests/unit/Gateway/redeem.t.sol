...
contract Redeem_Test is Gateway_Base_Test {
    ...
    function test_redeem_Success() public {
        uint256 shares = elitraVault.balanceOf(users.bob);
        ...
        uint256 assetsOrRequestId = gateway.redeem(address(elitraVault), shares, MIN_ASSETS_OUT, users.bob, PARTNER_ID);
    ...
    function test_redeem_EmitsEvent() public {
        ...
        emit ElitraGatewayRedeem(
            PARTNER_ID,
            address(elitraVault),
            users.bob,
            shares,
            elitraVault.previewRedeem(shares),
            true // instant
        );

        gateway.redeem(address(elitraVault), shares, MIN_ASSETS_OUT, users.bob, PARTNER_ID);
    ...
    function test_redeem_RevertWhen_ZeroAmount() public {
        ...
        gateway.redeem(address(elitraVault), 0, MIN_ASSETS_OUT, users.bob, PARTNER_ID);
    ...
    function test_redeem_RevertWhen_ZeroReceiver() public {
        ...
        gateway.redeem(address(elitraVault), shares, MIN_ASSETS_OUT, address(0), PARTNER_ID);
    ...
    function test_redeem_RevertWhen_InsufficientAssetsOut() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                Errors.Gateway__InsufficientAssetsOut.selector, elitraVault.previewRedeem(shares), veryHighMinAssets
            )
        );

        gateway.redeem(address(elitraVault), shares, veryHighMinAssets, users.bob, PARTNER_ID);
    ...
    function test_redeem_RevertWhen_InsufficientAllowance() public {
        ...
        // Revoke allowance
        elitraVault.approve(address(gateway), 0);
    ...
    function test_redeem_RevertWhen_InsufficientBalance() public {
        ...
        gateway.redeem(address(elitraVault), largeShares, MIN_ASSETS_OUT, users.bob, PARTNER_ID);
