...
contract Allowance_Test is Gateway_Base_Test {
    ...
    function test_getShareAllowance_Success() public {
        ...
        uint256 allowance = gateway.getShareAllowance(address(elitraVault), users.bob);
    ...
    function test_getAssetAllowance_Success() public {
        ...
        uint256 allowance = gateway.getAssetAllowance(address(elitraVault), users.bob);
    ...
    function test_allowanceFunctions_WithDifferentUsers() public {
        ...
        elitraVault.approve(address(gateway), 2000e18);
        usdc.approve(address(gateway), 1500e6);
        vm.stopPrank();

        // Check Bob's allowances
        uint256 bobShareAllowance = gateway.getShareAllowance(address(elitraVault), users.bob);
        uint256 bobAssetAllowance = gateway.getAssetAllowance(address(elitraVault), users.bob);

        // Check Alice's allowances
        uint256 aliceShareAllowance = gateway.getShareAllowance(address(elitraVault), users.alice);
        uint256 aliceAssetAllowance = gateway.getAssetAllowance(address(elitraVault), users.alice);
