...
// src/interfaces/IElitraVault.sol
...
// src/interfaces/IRedemptionHook.sol
...
contract ElitraVault is ERC4626Upgradeable, Compatible, IElitraVault, AuthUpgradeable, PausableUpgradeable {
    ...
    function initialize(
        ...
        require(address(_balanceUpdateHook) != address(0), Errors_1.ZeroAddress());
        require(address(_redemptionHook) != address(0), Errors_1.ZeroAddress());
    ...
    function updateBalance(uint256 newAggregatedBalance) external requiresAuth {
        ...
        require(block.number > lastBlockUpdated, Errors_1.UpdateAlreadyCompletedInThisBlock());
    ...
    function setBalanceUpdateHook(IBalanceUpdateHook newAdapter) external requiresAuth {
        require(address(newAdapter) != address(0), Errors_1.ZeroAddress());
    ...
    function requestRedeem(uint256 shares, address receiver, address owner)
        ...
        returns (uint256)
    {
        require(shares > 0, Errors_1.SharesAmountZero());
        require(owner == msg.sender, Errors_1.NotSharesOwner());
        require(balanceOf(owner) >= shares, Errors_1.InsufficientShares());
        ...
        if (mode == RedemptionMode.INSTANT) {
            ...
            revert Errors_1.InvalidRedemptionMode();
        }
    }
    ...
    function fulfillRedeem(address receiver, uint256 shares, uint256 assets) external requiresAuth {
        PendingRedeem storage pending = _pendingRedeem[receiver];
        require(pending.shares != 0 && shares <= pending.shares, Errors_1.InvalidSharesAmount());
        require(pending.assets != 0 && assets <= pending.assets, Errors_1.InvalidAssetsAmount());
    ...
    function setRedemptionHook(IRedemptionHook newStrategy) external requiresAuth {
        require(address(newStrategy) != address(0), Errors_1.ZeroAddress());
        ...
        returns (bytes memory result)
    {
        ...
        require(
            ...
            Errors_1.TargetMethodNotAuthorized(target, functionSig)
        );
        ...
        requiresAuth
    {
        ...
        for (uint256 i = 0; i < targets.length; i++) {
            ...
            require(
                ...
                Errors_1.TargetMethodNotAuthorized(targets[i], functionSig)
            );
    ...
    function withdraw(uint256, address, address)
        ...
        returns (uint256)
    {
        revert Errors_1.UseRequestRedeem();
    }
